...
import { down, up } from "./013.attachment.js";
...
describe("013 attachment migration", () => {
  ...
  async function seedTree() {
    ...
    await db
      ...
      .values({
        ...
        mime_type: "image/png",
        size_bytes: 123,
        storage_key: `cards/${card.id}/${crypto.randomUUID()}.png`,
      })
      .execute();
    return { card, user };
  }
  ...
  it("creates the attachments table and inserts a row", async () => {
    await seedTree();
    expect(await count()).toBe(1);
  });

  it("cascades attachment rows when the parent card is deleted", async () => {
    const { card } = await seedTree();
    await db.deleteFrom("cards").where("id", "=", card.id).execute();
    expect(await count()).toBe(0);
  });

  it("cascades attachment rows when the uploader user is deleted", async () => {
    const { user } = await seedTree();
    await db.deleteFrom("users").where("id", "=", user.id).execute();
    expect(await count()).toBe(0);
  });

  it("down drops the attachments table", async () => {
    await down(db);
    await expect(
      sql`select 1 from ${sql.table("attachments")}`.execute(db),
    ).rejects.toThrow();
  });
});
